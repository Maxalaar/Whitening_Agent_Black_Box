...
class LatentSpaceVisualisation:
    ...
    def plot(self):
        ...
        #     display_type = change.new
        #     self.plot_cluster_rendering_fusions(display_type)

        # Creating the dropdown box
        selection_dropdown = widgets.Dropdown(
            options=[('Display all images together', 'ensemble'), ('Display images individually', 'individual')],
            value='ensemble',
            description='Display Type:',
            disabled=False,
        )
        display(selection_dropdown)
        widgets.interactive(self.plot_cluster_rendering_fusions, option=selection_dropdown)

        # Linking the callback function to the dropdown box
        # selection_dropdown.observe(update_plot, names='value')
        #
        # # Displaying the dropdown box
        # display(selection_dropdown)

        cloud_dots_figure.canvas.mpl_connect('motion_notify_event', on_hover)
        plt.show()

    def plot_cluster_rendering_fusions(self, display_type='ensemble'):

        nb_images = len(self.cluster_average_rendering)

        if display_type == 'ensemble':
            nb_cols = 4
        ...
        elif display_type == 'individual':
            cluster_rendering_fusions_figure, cluster_rendering_fusions_axes = plt.subplots(1, 1, figsize=(6, 4))
            cluster_rendering_fusions_figure.canvas.manager.set_window_title('Cluster Rendering Fusions')
            cluster_rendering_fusions_axes.imshow(self.cluster_average_rendering[0] / 255.0, cmap='gray')
            cluster_rendering_fusions_axes.axis('off')

        else:
            raise ValueError("Invalid display type. Use 'ensemble' or 'individual'.")
